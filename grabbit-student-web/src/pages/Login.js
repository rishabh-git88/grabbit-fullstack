    ...
    try {
      ...
      if (res.data.success) {
        if (res.data.user.role !== 'student') {
          toast.error('This is a student portal. Use vendor dashboard instead.');
          return;
        }
      ...
      toast.error(err.response?.data?.message || 'Account not found. Please register.');
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f1117 0%, #1a1d2e 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      <div style={{ background: '#1a1d2e', borderRadius: 20, padding: '40px 36px', width: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.5)', border: '1px solid #2d3148' }}>
        ...
        <div style={{ display: 'flex', background: '#0f1117', borderRadius: 12, padding: 4, marginBottom: 28, border: '1px solid #2d3148' }}>
          {[{ key: 'google', label: '📧 Gmail' }, { key: 'phone', label: '📱 Mobile OTP' }].map((tab) => (
            <button key={tab.key} onClick={() => handleModeSwitch(tab.key)} style={{ flex: 1, padding: '10px 0', border: 'none', borderRadius: 9, cursor: 'pointer', fontSize: 14, background: mode === tab.key ? 'linear-gradient(135deg, #f97316, #ea580c)' : 'transparent', color: mode === tab.key ? '#fff' : '#6b7280', fontWeight: mode === tab.key ? 600 : 400 }}>{tab.label}</button>
          ))}
        </div>

        {mode === 'google' && (
          <div>
            <p style={{ color: '#9ca3af', fontSize: 14, textAlign: 'center', marginBottom: 24 }}>Sign in with your college Gmail account</p>
          ...
          <div>
            {step === 1 && (
              <>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                  ...
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))} placeholder="98765 43210" style={{ flex: 1, padding: '12px 16px', borderRadius: 10, background: '#0f1117', border: '1px solid #374151', color: '#fff', fontSize: 15, outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'center' }}><div id="recaptcha-container"></div></div>
                <button onClick={handleSendOTP} disabled={loading || !recaptchaSolved} style={{ width: '100%', padding: '14px 0', borderRadius: 12, background: (!recaptchaSolved || loading) ? '#374151' : 'linear-gradient(135deg, #f97316, #ea580c)', border: 'none', color: '#fff', fontSize: 16, fontWeight: 600, cursor: (!recaptchaSolved || loading) ? 'not-allowed' : 'pointer' }}>
            ...
            {step === 2 && (
              <>
                ...
                <input type="text" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="• • • • • •" maxLength={6} style={{ width: '100%', padding: '16px', borderRadius: 10, background: '#0f1117', border: '1px solid #374151', color: '#fff', fontSize: 24, textAlign: 'center', letterSpacing: 12, outline: 'none', boxSizing: 'border-box', marginBottom: 20 }} />
                <button onClick={handleVerifyOTP} disabled={loading} style={{ width: '100%', padding: '14px 0', borderRadius: 12, background: loading ? '#374151' : 'linear-gradient(135deg, #f97316, #ea580c)', border: 'none', color: '#fff', fontSize: 16, fontWeight: 600, cursor: 'pointer', marginBottom: 12 }}>{loading ? 'Verifying...' : 'Verify OTP ✓'}</button>
                <button onClick={() => { setStep(1); setOtp(''); clearRecaptcha(); }} style={{ width: '100%', padding: '12px 0', borderRadius: 12, background: 'transparent', border: '1px solid #374151', color: '#6b7280', fontSize: 14, cursor: 'pointer' }}>← Change Number</button>
              </>
            )}
          </div>
        )}
